	...
	public static void main(String[] args) {
		try {
			...
			sql = "CREATE TABLE items (item_id VARCHAR(255) NOT NULL, "
					+ "name VARCHAR(255), rating FLOAT, address VARCHAR(255), "
					+ "image_url VARCHAR(255), url VARCHAR(255), distance FLOAT, "
					+ "PRIMARY KEY (item_id))";
			stmt.executeUpdate(sql);
			...
			sql = "CREATE TABLE users (user_id VARCHAR(255) NOT NULL, "
					...
					+ "lastname VARCHAR(255), PRIMARY KEY (user_id))";
			stmt.executeUpdate(sql);
			
			sql = "CREATE TABLE history (user_id VARCHAR(255) NOT NULL, "
					+ "item_id VARCHAR(255) NOT NULL, last_favor_time timestamp, "
					+ "PRIMARY KEY (user_id, item_id), FOREIGN KEY (user_id) REFERENCES users(user_id), "
					+ "FOREIGN KEY (item_id) REFERENCES items(item_id))";
			stmt.executeUpdate(sql);
			
			// Insert a user
			sql = "INSERT INTO users VALUES (\"1111\", \"3229c1097c00d497a0fd282d586be050\",\"Yang\", \"Mi\")";
			stmt.executeUpdate(sql);
